...
nixlRemoteSection::loadRemoteData(nixlSerDes *deserializer, backend_map_t &backendToEngineMap) {
    ...
    for (size_t i = 0; i < seg_count; ++i) {
        ...
        if (nixl_backend.empty()) {
            // v86: A peer can advertise fewer backends than seg_count claims while it is
            // still bringing rails up. Stop consuming instead of failing the whole load.
            break;
        }

        nixl_reg_dlist_t s_desc(deserializer);
        if (s_desc.isEmpty()) { // can be used for entry removal in future
            // v81: An agent may publish metadata before its KV cache is fully allocated,
            // yielding an empty list for a backend. Skip it; a later load picks it up.
            continue;
        }
